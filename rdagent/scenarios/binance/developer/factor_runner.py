...
class BinanceFactorRunner(CachedRunner[BinanceFactorExperiment]):
    """
    Docker run
    ...
    def develop(self, exp: BinanceFactorExperiment) -> BinanceFactorExperiment:
        """
        ...
        fbps = BinanceFactorBasePropSetting()
        env_to_use = {
            "PYTHONPATH": "./",
        ...
        if exp.based_experiments:
            SOTA_factor = None
            # Filter and retain only BinanceFactorExperiment instances
            sota_factor_experiments_list = [
                base_exp for base_exp in exp.based_experiments if isinstance(base_exp, BinanceFactorExperiment)
            ]
